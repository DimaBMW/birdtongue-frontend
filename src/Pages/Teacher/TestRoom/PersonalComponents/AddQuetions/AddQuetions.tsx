...
const AddQuetions: React.FC<InProps> = ({
  ...
  const onChangeFormChossing=(event: React.ChangeEvent<HTMLInputElement>)=>{
    setValue('ChoosingAnAnswerForm',event.target.value);
  }
  const submit: SubmitHandler<InQuetionForm> = (data) => {
    console.log(data);
  };
  const WatchInputFieldForm= watch('InputFieldForm', '');
  const WatchChoossingAnswerForm=watch('ChoosingAnAnswerForm','');
            ...
            {(() => {
              switch (currentMenu) {
                ...
                case "choosingananswer":
                    return(<ChoosingAnAnswer onChange={onChangeFormChossing} value={WatchChoossingAnswerForm}/>);
