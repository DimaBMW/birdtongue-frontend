// Стили
import styles from "./Add_Movie_Form.module.scss";
// Бибилиотеки
...
const AddMovieForm:React.FC<InAdminForm>=({isActoveForm})=>{
    //необходимые методы для useFrom
    const {register,handleSubmit,formState:{errors}}=useForm<InAdminForm>();
    // Обрабатываем данные с формы
    const submit:SubmitHandler<InAdminForm>=data=>{data}
    ...
    !isActoveForm === closeAdminForm?document.body.classList.add("no-scroll"):document.body.classList.remove('no-scroll')
    ...
    return(
        <div className={`${!isActoveForm === closeAdminForm?styles.form_container_active:styles.form_container}`}>
            <form className={styles.form} onSubmit={handleSubmit(submit)}>
                <div className={styles.form_title}>
                    <h1>Adding a movie</h1>
                    <div className={styles.form_title_icon}>
                        <button onClick={handleMaximazeAdminForm}><Image src={!maximizeAdminForm?maximize:minimize} alt="увеличить" className="form-admin__icon"/></button>
                        <button onClick={handleCloseAdminForm}><Image src={close} alt="закрыть" className="form-admin__icon"/></button>
                    </div>
                </div>
                <div className={`${styles.form_main} ${maximizeAdminForm?styles.fomrmax:''}`}>
                    <div className={styles.form_main_linktoMovieFile}>
                        ...
                        <div className={styles.form_main_linktoMovieFile_btnaddfile}>
                            <input type="file" id="file-input" style={{display:"none",visibility:"hidden"}} onChange={handleSelectedFile}/>
                            <button onClick={()=>document.getElementById('file-input')?.click()} className={styles.btn_adminForm}>Review</button>
                            <h1>{selectdFile||""}</h1>
                        </div>
                    </div>
                    ...
                    <div className={styles.form_main_selectsbox}>
                        <div className={styles.form_main_selectsbox__rows1}>
                            <FormSelect options={options} name="Language" currentSelect={currentSelectLanguage} onChange={handleOnChangeLanguage}/>
                            <FormSelect options={LevelOption} name="Language level" currentSelect={currentSelectLevel} onChange={handleOnChangeLevel}/>
                        </div>
                        <div className={styles.form_main_selectsbox_rows2}>
                            ...
                            <div className={styles.form_main_selectsbox_rows2_fileSubptatile}>
                                <FormSelect options={LanguageSubtitles} name="Subtitle language" currentSelect={currentSelectSubtitle} onChange={handleOnChangeSubtitle}/>
                                <div className={styles.filebtnSubtitle}>
                                    <input type="file" id="file-input2" style={{display:"none",visibility:"hidden"}} onChange={handleSelectedFile2}/>
                                    <button onClick={()=>document.getElementById('file-input2')?.click()} className={styles.btn_adminForm}>Review</button>
                                    <h1>{selectdFile2||""}</h1>
                                </div>
                            </div>
                            <div className={styles.formbtnsubtitles}>
                                <ButtonOne name="Add subtitles" icon={1} className="admin-opacity"/>
                            </div>
                        </div>
                    </div>
